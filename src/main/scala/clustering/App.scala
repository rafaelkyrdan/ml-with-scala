  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      conf = Some(new SparkConf()
        .setMaster("local[2]")
        .setAppName("Classification. K-means")
        .set("spark.app.id", "classification"))
      sc = Some(new SparkContext(conf.get))

      // load movie data
      val movies = sc.get.textFile("./data/ml-100k/u.item")
      println(movies.first)
      val genres = sc.get.textFile("./data/ml-100k/u.genre")
      ...
      // this RDD contains movie ID, title, and genres
      val titlesAndGenres = movies.map(_.split("\\|")).map { array =>
        val genres = array.toSeq.slice(5, array.length)
      ...
      // RDDs with userFeatures and productFeatures
      val rawData = sc.get.textFile("./data/ml-100k/u.data")
      ...
      val trainTestSplitUsers2 = userVectors.randomSplit(Array(0.6, 0.4), 123)
      val trainUsers2 = trainTestSplitUsers2(0)
      val testUsers2 = trainTestSplitUsers2(1)
      val costsUsers2 = Seq(2, 3, 4, 5, 10, 20).map { k => (k, KMeans.train(trainUsers2, numIterations, k, numRuns).computeCost(testUsers2)) }
      println("User clustering cross-validation:")
      costsUsers2.foreach { case (k, cost) => println(f"WCSS for K=$k id $cost%2.2f") }
